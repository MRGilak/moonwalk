...
We learned a method to bias and turn on our transistor in [Fixing the Base Current](/notes/Electronics/Fixing the Base Current/). The problem with that method is that it is very device-dependent. Basically, if we change our device, its $\beta$ changes and therefore fixing $I_B$ does not lead to the same $I_C$ for every device. This means that the method is not _repeatable_. Different transistors of the same model might have different $\beta$s (in a range). In this note, we want to learn another useful method to bias the transistor that is less device-dependent.
Instead of fixing the base current, we would like to fix the collector current itself. Because we have $I_C \approx I_E$, we can fix the emitter current instead. If we place a resistor in the emitter, fixing the emitter current is equivalent to fixing the emitter voltage. Assuming a $V_{BE_{on}}$ for the device, we fix the base voltage $V_B$ instead. Note that this is different than the solution we rejected [here](/notes/Electronics/Biasing the Transistor/). Here, we are not fixing $V_{BE}$, but $V_B$ instead.
We can write
...
Continue learning about electronics [here](/notes/Electronics/Common Collector Structure/).