...
The dynamic port behavior of a robot is a property of the robot itself and does not depend on the environment anymore, but motion and force depend on both the robot and the environment. Basically, the idea behind impedance control is if arbitrary impedance can be achieved, arbitrary performance can be achieved.
### What this means for a single leg robot?
For the single leg robot that we have in [this project](/notes/Robotics/Single Leg Project/Single Leg Project/), impedance control seems like an appropriate approach. Controlling only motion could lead to arbitrarily large torques at the robot joints when it is interacting with the ground.
...
where $\tau_{ff}$ is the feed-forward torque. When the foot is on the ground, it can be calculated as

$$
\tau_{ff} = J^T [ K_P (x - L(q)) + K_D (\dot{x} - J(q) \dot{q}) ]
$$

Basically $x$ is compared with $L(q)$ and $\dot{x}$ is compared to $J(q) \dot{q}$. This displacement and difference in velocity is then transformed into force by considering a desired mechanical behavior for the robot in the form of $K_P$ and $K_D$. It is very important to note that although this looks like a PD controller, the intuition behind it is very different. The coefficients $K_P$ and $K_D$ represent a spring-damper behavior for the robot. The resulting force is then converted to torques using the transpose of the Jacobian matrix, $J^T$. This $\tau_{ff}$ torque is then passed to a PD controller, because feed-forward-only control would degrade performance (usually because the internal PD controller of a motor works at a much higher frequency).
