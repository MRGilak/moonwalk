...
Finally, the constraint on the system is that its foot cannot penetrate the ground, which can be added as a Lagrange multiplier.
...
$$
	\mathcal{L} = K + P + \lambda c.
$$
We can calculate the dynamics of the joint angles by solving
$$
	\begin{align}
		\frac{\partial \mathcal{L}}{\partial q_1} - \frac{\mathrm{d}}{\mathrm{d} t} \frac{\partial \mathcal{L}}{\partial \dot{q_1}} = \tau_1 \\
		\frac{\partial \mathcal{L}}{\partial q_2} - \frac{\mathrm{d}}{\mathrm{d} t} \frac{\partial \mathcal{L}}{\partial \dot{q_2}} = \tau_2
	\end{align}
$$
...
$$
When the foot is on the ground, the force $F$ can be calculated using $\lambda$.
