...
date: 2025-10-16
excerpt: "Stability of legged robots is analyzed through concepts like fixed points and limit cycles. These concepts relate to the robots ability to maintain safe postures and motions."
---
...
# Fixed Points
Fixed point in the context of robotics apparently refers to equilibrium points. For a system with dynamics $\dot{x} = f(x)$, a point $x^\ast$ is called a fixed point if $f(x^\ast) = 0$. For the model of a legged robot that we have in [this note](/notes/Robotics/Single Leg Project/Robot Leg Equations/), a fixed point is a configuration $q^\ast$ for which $\dot{q^\ast} = 0$ and $\ddot{q^\ast} = 0$ is a solution to the equation. For a fixed point we'll then have $\ddot{c} = 0$ and $\dot{L}$ = 0 which in turn gives
$$
